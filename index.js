...
export default class BlinkIDReactNativeApp extends Component {
    ...
    async scan() {
        try {
            var germanIdRecognizer = new BlinkIDReactNative.GermanyIdFrontRecognizer();
            germanIdRecognizer.returnFullDocumentImage = true;
            germanIdRecognizer.returnFaceImage = true
            var germanIdSuccessFrameGrabber = new BlinkIDReactNative.SuccessFrameGrabberRecognizer(germanIdRecognizer);
            ...
            const scanningResults = await BlinkIDReactNative.BlinkID.scanWithCamera(
                ...
                new BlinkIDReactNative.RecognizerCollection([germanIdSuccessFrameGrabber, eudlSuccessFrameGrabber, usdlSuccessFrameGrabber, mrtdSuccessFrameGrabber]),
                licenseKey
            );

            console.log('RESULTTTT', scanningResults)
            if (scanningResults) {
                let newState = {
                    showImageDocument: false,
                    resultImageDocument: '',
...
AppRegistry.registerComponent(appName, () => BlinkIDReactNativeApp);
